...
export function RevokeDialog({ version, open, onOpenChange, onDone }: { version: string; open: boolean; onOpenChange: (open: boolean) => void; onDone: () => void | Promise<void> }) {
  ...
  return <AlertDialog open={open} onOpenChange={(next) => { if (!busy) onOpenChange(next); }}>
    <AlertDialogContent>
      <AlertDialogHeader>
        <AlertDialogTitle>撤销版本 {version}？</AlertDialogTitle>
      ...
      <div className="grid gap-2">
        ...
        <Textarea id="revoke-reason" rows={3} maxLength={500} value={reason} disabled={busy} placeholder="例如：启动后无法进入主界面，已回滚" onChange={(event) => setReason(event.target.value)} />
        <p className="text-xs text-muted-foreground">操作人会记录为当前 Cloudflare Access 身份，无法在此修改。</p>
      </div>
      {error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
      <AlertDialogFooter>
        <AlertDialogCancel disabled={busy}>取消</AlertDialogCancel>
        <Button variant="destructive" disabled={busy} onClick={() => void submit()}><ShieldOff />{busy ? "正在撤销…" : "确认撤销"}</Button>
      </AlertDialogFooter>
    </AlertDialogContent>
  </AlertDialog>;
}
...
export function RestoreDialog({ version, open, onOpenChange, onDone }: { version: string; open: boolean; onOpenChange: (open: boolean) => void; onDone: () => void | Promise<void> }) {
  ...
  return <AlertDialog open={open} onOpenChange={(next) => { if (!busy) onOpenChange(next); }}>
    <AlertDialogContent>
      ...
      <AlertDialogFooter>
        <AlertDialogCancel disabled={busy}>取消</AlertDialogCancel>
        <Button disabled={busy} onClick={() => void submit()}><RotateCcw />{busy ? "正在恢复…" : "确认恢复"}</Button>
      </AlertDialogFooter>
    </AlertDialogContent>
  </AlertDialog>;
}
