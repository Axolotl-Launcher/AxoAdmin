...
export default function Updates() {
  ...
  return (
    <div className="grid min-w-0 gap-6">
      ...
      {versions.loading && !versions.data && <AdminLoading label="正在加载版本列表…" />}

      {versions.data && (
        <Card className="min-w-0">
          <CardHeader>
            <CardTitle>版本列表</CardTitle>
            <CardDescription>按 SemVer 倒序排列。上游仅保留最新 3 个 Release 与 3 个 Beta 的产物，更早版本的下载会自动跳转到 GitHub Release。</CardDescription>
          </CardHeader>
          <CardContent className="grid min-w-0 gap-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <Select value={channel} onValueChange={setChannel}>
                <SelectTrigger className="w-32" aria-label="渠道"><SelectValue /></SelectTrigger>
                <SelectContent>
              ...
              <Select value={status} onValueChange={setStatus}>
                <SelectTrigger className="w-32" aria-label="状态"><SelectValue /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">全部状态</SelectItem>
                  {Object.entries(statusNames).map(([value, name]) => <SelectItem key={value} value={value}>{name}</SelectItem>)}
                </SelectContent>
              </Select>
              {(search || channel !== "all" || status !== "all") && <Button variant="ghost" size="sm" onClick={() => { setSearch(""); setChannel("all"); setStatus("all"); }}>重置筛选</Button>}
              <span className="ml-auto text-xs text-muted-foreground">{rows.length} / {list.length} 个版本</span>
            </div>

            {rows.length === 0 ? (
              <AdminEmpty label={list.length === 0 ? "更新服务还没有任何版本记录。" : "没有符合当前筛选条件的版本。"} hint={list.length === 0 ? "发布流程上报后会自动出现在这里。" : "调整关键词或筛选条件后重试。"} />
            ) : (
              <Table>
                <TableHeader>
                  <TableRow>
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" aria-label={"管理版本：" + item.version}><MoreHorizontal /></Button>
                          </DropdownMenuTrigger>
      ...
      <RevokeDialog version={revoking ?? ""} open={Boolean(revoking)} onOpenChange={(open) => { if (!open) setRevoking(null); }} onDone={reload} />
      <RestoreDialog version={restoring ?? ""} open={Boolean(restoring)} onOpenChange={(open) => { if (!open) setRestoring(null); }} onDone={reload} />
    </div>
  );
}
