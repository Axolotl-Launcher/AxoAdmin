  ...
  return (
    <div className="grid min-w-0 gap-6">
      <PageHeader title="发布审计" description="由更新服务记录的版本撤销与恢复操作，用于追溯一次发布被下线的原因。">
        <Button variant="outline" disabled={audit.loading} onClick={audit.reload}><RefreshCw className={audit.loading ? "animate-spin" : undefined} />刷新</Button>
      </PageHeader>

      {audit.error && <AdminError message={audit.error} onRetry={audit.reload} />}
      {audit.loading && !audit.data && <AdminLoading label="正在加载发布审计…" />}

      <Card className="min-w-0">
        <CardHeader>
          <CardTitle>撤销与恢复记录</CardTitle>
          <CardDescription>只包含更新服务的撤销与恢复操作；浏览器侧的对应操作同时会写入平台审计日志。下载量请见「下载统计」。</CardDescription>
        </CardHeader>
        <CardContent className="min-w-0">
          {audit.data && logs.length === 0 ? <AdminEmpty label="还没有任何撤销或恢复记录。" hint="发布版本并触发撤销后，记录会出现在这里。" /> : logs.length > 0 && (
            <Table>
              <TableHeader>
                <TableRow>
