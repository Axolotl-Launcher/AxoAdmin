...
const allowed = new Set(["overview", "activity", "distributions", "system"]);
...
export async function telemetryRequest(path: string, request: Request) {
  ...
  if (parts.length > 1) return failure("Not found", 404);
  if ([...incoming.searchParams.keys()].some((key) => key !== "range")) return failure("Invalid query parameters", 400);
